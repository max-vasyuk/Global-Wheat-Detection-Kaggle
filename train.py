...
class Trainer:
    ...
    def fit(self, train_loader, validation_loader):
        for e in range(self.config.n_epochs):
            ...
            self.save(f'{self.base_dir}/last-checkpoint.pt')

            t = time.time()
            summary_loss = self.validation(validation_loader)

            self.log(
                f'Val. Epoch: {self.epoch}, summary_loss: {summary_loss.avg:.5f}, time: {(time.time() - t):.5f}')
            if summary_loss.avg < self.best_summary_loss:
                self.best_summary_loss = summary_loss.avg
                self.model.eval()
                self.save(f'{self.base_dir}/best-checkpoint-{str(self.epoch).zfill(3)}epoch.pt')
                for path in sorted(glob(f'{self.base_dir}/best-checkpoint-*epoch.pt'))[:-3]:
                    os.remove(path)
...
def run_training(net, train_dataset, validation_dataset):
    ...
    fitter = Trainer(model=net, device=device, config=config)
